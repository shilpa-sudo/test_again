...
for tx in range (0,2000):
    ...
    if  areas == [] or areas == None : #means there are no mhw polygons on this day
        ...
        xrds.to_netcdf(f'/home2/datahome/slal/polygon_areas/mhw_polygon_areas_{tx}.nc')
        ...
        for i in range(len(polygonslist)):
            ...
            for j in range(len(polygonslist)):
                if i ==j:
                    ...
                    if not polygonslist[i].is_valid:
                        polygonslist[i] = polygonslist[i].buffer(0)
                    if not polygonslist[j].is_valid:
                        polygonslist[j] = polygonslist[j].buffer(0)

                    if polygonslist[i].intersects(polygonslist[j]):
                        intersectarea = polygonslist[i].intersection(polygonslist[j]).area
                        ...
                        intersectionarea_list.append(0)
