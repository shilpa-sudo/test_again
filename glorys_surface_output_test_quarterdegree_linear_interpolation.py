...
fnames = glob.glob('/home/datawork-lead/datarmor-only/shilpa/glorys_quarter_datarmor/*.nc')  
fnames.sort()
...
#ds = xr.open_mfdataset(filelist[12:], combine='nested',concat_dim="time",preprocess=partial_func)  
...
spatial_extent = np.zeros((9861, 141, 604))
#date_start_arr = np.zeros((9861, 144, 588))
#date_end_arr = np.zeros((9861, 144, 588))
#date_peak_arr = np.zeros((9861, 144, 588))
index_start_arr = np.zeros((9861, 141, 604))
index_end_arr = np.zeros((9861, 141, 604))
index_peak_arr = np.zeros((9861, 141, 604))
duration_arr = np.zeros((9861, 141, 604))
intensity_max_arr = np.zeros((9861, 141, 604))
intensity_mean_arr = np.zeros((9861, 141, 604)) 
intensity_cumulative_arr = np.zeros((9861, 141, 604))
#category_arr = np.zeros((9861, 144, 588))
rateonset_arr = np.zeros((9861, 141, 604))
ratedecline_arr = np.zeros((9861, 141, 604))

intensity_t_minus_clim_arr = np.zeros((9861, 141, 604)) 
intensity_t_minus_thresh_arr = np.zeros((9861, 141, 604))

reaction_window_days_arr = np.zeros((9861, 141, 604))
coping_window_days_arr = np.zeros((9861, 141, 604))
recovery_window_days_arr = np.zeros((9861, 141, 604))

severity_index_arr = np.zeros((9861, 141, 604))
no_mhw_events_arr = np.zeros((9861, 141, 604))
...
for j in range(141):
    for k in range(604):
        ...
        intensity_t_minus_clim_arr[:,j,k] = temparr[:,j,k] - clim['seas'][:]  #intensity_t_minus_clim       # array is filled here
...
longi = np.array(ds.nav_lon.isel(y=10).values)
lati = np.array(ds.nav_lat.isel(x=10).values)
time = pd.date_range(start='1993-01-01', end='2019-12-31')
...
xrds.to_netcdf('/home2/datawork/slal/glorys_mhw_output_surface_linear_intensity_test.nc')
